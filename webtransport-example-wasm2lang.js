...
async function loadWasm(wasmUrl, certDerUrl, keyDerUrl) {
  const [{module, memory}, certDer, keyDer] = await Promise.all([
    import(wasmUrl).then(({default:module, memory}) => ({ module, memory })),
    readFile(certDerUrl),
    readFile(keyDerUrl),
  ]);
  ...
  */
  ...
  // const buffer = new ArrayBuffer(1024**2*10);
  const wasm = module(null,
    env
  );
  wasm.memory = memory;
  // console.log(wasm);
...
function copyToWasm(wasm, data, fn) {
  // console.log(data);
  if (!data) {
     console.log("No data, WTF?");
     return;
     throw new Error("dconsole.logata ain't defined! WTH?");
   // data = wasm.memory.buffer;
  };
...
function drainPackets(wasm) {
  ...
  console.log("[drain] qz_send_packets -> " + total + "B");
...
async function main({
  ...
  const wasm = await loadWasm("./quic.js", "./cert.der", "./key.der");
  wasm.qz_init_server();
  ...
  const handlers = {
    ...
    onStreamData(streamId, length) {
      ...
      //   └─────────────────────────────────────────────────┘
      const data = readStreamData(wasm, streamId);
      //console.log(data);
      const [chunks] = data;
...
class DirectSocketsUDPSocket {
  ...
  constructor(options = {}) {
    ...
    this.#socket = createSocket({
      type,
      reuseAddr: false // options.reuseAddr || options.multicastAllowAddressSharing,
    });
