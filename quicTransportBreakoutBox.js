async function quicTransportBreakoutBox(text) {
  ...
  try {
    ...
    const data = encoder.encode(`espeak-ng -m --stdout "${text}"`);
    await writer.write(data);
    await writer.close();
    const reader = transport.receiveStreams().getReader();
    const result = await reader.read();
    const transportStream = result.value;
    console.log({ sender, writer, reader, transportStream });
    ...
    osc.connect(msd);
    osc.start();
    console.log(track.getSettings(), generator.getSettings());
    ...
    await Promise.all([
      ...
      audioReader.read().then(async function process({ value, done }) {
        ...
        if (audio.currentTime < value.buffer.duration * 100) {
        ...
        if (writeOffset > readOffset) {
          ...
          if (audio.currentTime < duration + value.buffer.duration * 200) {
